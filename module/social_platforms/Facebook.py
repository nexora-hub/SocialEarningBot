...
class FB:
      def __init__(self, instance):
          ...
          self.account_credentials = instance.data.get("Facebook", {})
      ...
      async def start(self, context):
            ...
            # Endpoint for fetching Facebook-specific tasks
            self.task_endpoint = self.instance.facebook_task_endpoint

            try:
              # Initialize session (cookies, state, login checks, etc.)
              if await self.session_initialization():
                 # Mark Facebook session as successfully synced
                 self.instance.sync_facebook = True
              else:
                 # Mark session as invalid → automation should not continue
                 self.instance.sync_facebook = False

                 # Log error message indicating session initialization failed
                 await self.drive.helper.print(f"\033[93m[SDE]\033[0m|\033[96mFacebook → session_initialization\033[0m(\033[91merror\033[0m): Opp's something went wrong")

                 # Exit early due to failed initialization
                 return False

              # Check if there are no tasks available for Facebook
              ...
              await self.task_extraction()
               ...
               await self.drive.helper.print(f"[\033[93mSDE\033[0m]|\033[96mFacebook → start\033[0m(\033[91merror\033[0m): {str(error)}")
      ...
      async def session_initialization(self):
            """
            Mounts the Facebook session by verifying authentication and accessing the homepage
            """

            try:
              # Initialize Facebook session attributes for version v1
              if not await self.mount_credentials('v1'):
                 return False

              # Initialize session components if not already initialized
              if not self.instance.sync_facebook:
              ...
              if await self.drive.helper.site_lamba(self.task_endpoint, self.page, silent=True):
                 return True  # Successfully mounted
                   ...
                   await self.drive.helper.print(f"[\033[93mSDE\033[0m]|\033[96mFacebook → session_initialization\033[0m(\033[91merror\033[0m): {str(error)}")

      async def mount_credentials(self, session_key):
            """
            Initializes and validates the Facebook session by verifying Media ID and required API credentials
            """

            # Prefix used for logging errors in a formatted style
            prefix = '[\033[93mSDE\033[0m]|\033[96mFacebook → mount_session_credentials\033[0m(\033[91merror\033[0m): '
            ...
            errors = {
             ...
             "missing_data": f"Missing Facebook session data for '{session_key}' →→ Please ensure the session information is provided",  # Error when session data is missing
             "invalid_id": [
                 "Media id is invalid!",                      # Error when Media ID is invalid
                 "Media id is required for your FB account!"  # Error when Media ID is not provided
              ]
            }
            ...
            try:
              ...
              if not session_data:
                 # Disable further Facebook processing until re-enabled externally
                 self.instance.facebook_disable = True
              ...
              if not self.media_id or not self.media_id.isdigit():
                 ...
                 # Log and exit if Media ID is invalid or missing
              ...
              return True
                   ...
                   await self.drive.helper.print(f"[\033[93mSDE\033[0m]|\033[96mInstagram → mount_session_credentials\033[0m(\033[91merror\033[0m): {str(error)}")

      async def account_lookup(self, prefix=f"{'=' * 26} [\033[91mALERT\033[0m] {'=' * 26}"):
            # Selectors to determine account and session state
            selectors = {
               "main": 'span[dir="auto"]:has-text("Settings & privacy")',
               "login": [
            ...
            errors = {
               "network_error": '[\033[93mSDE\033[0m]|\033[96mFacebook → authentication\033[0m(\033[91merror\033[0m): internet issues',
               "session_error": f"\n{prefix}\n\033[4m\033[91mAccess Denied!\033[0m\033[0m Manual authentication is required — log in to your FB account to proceed"
            }

            # Attempt to load account settings page to confirm session
            if not await self.drive.helper.site_lamba('https://www.facebook.com/settings', self.page, silent=True, timeout=20):
            ...
            auth = await asyncio.gather(
                ...
                self.drive.await_selectors('a[href="https://www.facebook.com/privacy/checkup/?source=settings"]', self.page, timeout=15, silent=True)  # Wait for inbox link selector
            )

            # If still on account settings page, consider session valid
            if any(auth) or self.page.url.startswith('https://www.facebook.com/settings'):
      ...
      async def detect_executable_tasks(self):
            """
            Scans the current page for known Facebook task types and determines if any are executable
            """

            # Check for the presence of each task type on the page
            for task in self.instance.facebook_task_types:
      ...
      async def is_task_available(self):
            ...
            if not await self.drive.helper.site_lamba(self.task_endpoint, self.page, silent=False, timeout=20):
               ...
               await self.drive.helper.print('[\033[93mSDE\033[0m]|\033[96mFacebook → is_task_available\033[0m(\033[91merror\033[0m): Poor internet')
            ...
            if self.page.url.startswith('https://socialearning.org/sign-in'):
               ...
               return await self.drive.helper.print(  # Print error message: user session expired or logged out
                  '\033[96mSocialEarning\033[0m(\033[93malert\033[0m): Detected redirection to login page — session may have expired or user was logged out'
               )
            ...
            if await self.drive.selector_inner_text("span[class='badge bg-primary'] span", self.page) == '0':
               ...
               return await self.drive.helper.print("[\033[93mSDE\033[0m]|\033[96mFacebook\033[0m(\033[91malert\033[0m): No tasks available!")
            ...
            for _ in range(2):
                ...
                for links in await self.drive.helper.extract_links(await self.page.content(), 'next=/earner/available/'):
                    ...
                    # Handle missing "AVAILABLE TASKS" indicator
                         ...
                         await self.drive.helper.print(f"\033[96mFacebook → view_task\033[0m(\033[91merror\033[0m): Required UI elements not found!")
      ...
      async def access_task(self, SE_task_link):
            # Verify Facebook automation & bot execution loop is enabled
            if self.instance.facebook_disable or not self.instance.infinite_loop:
               return False
            ...
            if not await self.drive.helper.site_lamba(SE_task_link, self.page, silent=True):
               ...
               await self.drive.helper.print('[\033[93mSDE\0330m]|\033[96mFacebook → access_task\033[0m(\033[91mnetwork\033[0m): slow internet')
            ...
            if not task_id:
               return await self.drive.helper.print('[\033[93mSDE\033[0m]|\033[96mFacebook → access_task\033[0m(\033[91mBadTaskId\033[0m): Invalid Task ID/Task Link')
      ...
      async def task_handler(self, task_id, SE_task_link, done: bool=None):
            ...
            task_workers = {
              'FACEBOOK/Page Like': self.page_like,
              'FACEBOOK/Page Follow': self.page_follow,
            }
            ...
            facebook_link = await self.drive.extract_selector_link(selectors[2], self.page)
            ...
            if not facebook_link.startswith('https://facebook.com') and not facebook_link.startswith('https://www.facebook.com'):
               return await self.instance.blacklist(SE_task_link, banned=True)

            # Check if the Facebook task URL is reachable
            if not await self.drive.helper.site_lamba(facebook_link, self.page, silent=True, timeout=30):
               return await self.drive.helper.print(f"[\033[93mSDE\033[0m]|\033[96mFacebook → task_handler\033[0m(\033[91merror\033[0m): Poor Internet")
            ...
            try:
              ...
              done = await worker() if task != 'FACEBOOK/Page Follow' else await worker(SE_task_link)
            except Exception as error:
                   # Log any exceptions raised by the worker
                   await self.drive.helper.print(f"[\033[93mSDE\033[0m]|\033[96mFacebook → task_handler → workers\033[0m(\033[91merror\033[0m): {str(error)}")
            ...
            if done:
               ...
               await self.drive.helper.schedule_tasks(task_id, self.media_id, 'Facebook')
      ...
      async def page_like(self):
            ...
            selector = {
               "main": [
                  'div[aria-label="Search"][role="button"]:has-text("Search")',
                  'div[role="button"][aria-haspopup="menu"][aria-label="Profile settings see more options"]'
               ],
               "like": 'div[class="xh8yej3"] div[aria-label="Like"][role="button"] span:has-text("Like")',
               "liked": 'div[class="xh8yej3"] div[aria-label="Liked"][role="button"] span:has-text("Liked")',
            }

            # Ensure Facebook UI is ready before proceeding
            if not await self.drive.await_selectors(selector['main'], self.page, silent=True):
               await self.drive.helper.print("[\033[93mSDE\033[0m]|\033[96mFacebook → Page_like\033[0m(\033[91merror\033[0m): slow_internet")

            # Try up to 3 times to click the "Like" button if it's visible
            if await self.drive.await_selectors(selector['like'], self.page, timeout=10, silent=True):
               for index in range(3):
                   # Click the like button (hovering to ensure visibility)
                   await self.drive.click_btn(selector['like'], self.page, silent=True, hover = True if index == 0 else False)

                   # Check if the like was successful (i.e., 'liked' icon appears)
                   if await self.drive.await_selectors(selector['liked'], self.page, timeout=3, silent=True):
                      break

            # Final check: confirm Page was successfully liked
            if await self.drive.await_selectors(selector['liked'], self.page, timeout=10, silent=True):
               return True

      async def page_follow(self, SE_task_link):
            # Prefix to prepend to error messages in logs
            error_prefix = "[\033[93mSDE\033[0m]|\033[96mFacebook → Page_follow\033[0m(\033[91merror\033[0m): "

            # Define necessary selectors for page verification
            selector = {
               "main": [
                  'div[role="button"][aria-label="Search"]:has-text("Search")',
                  'div[role="button"][aria-haspopup="menu"][aria-label="Profile settings see more options"]'
               ],
               "follow": 'div[class="xh8yej3"] div[aria-label="Follow"][role="button"]:has-text("Follow")',
               "following": 'div[class="xh8yej3"] div[aria-label="Following"][role="button"]:has-text("Following")',
            }

            # Ensure Facebook UI is ready before proceeding
            if not await self.drive.await_selectors(selector['main'], self.page, silent=True):
               # Check if the page is unavailable or just missing UI components
               if await self.drive.selectors_exists("text='This content isn't available at the moment'", self.page):
                  # Mark user as banned
                  await self.instance.blacklist(SE_task_link, banned=True)

                  # Log failure reason and exit
                  return False, await self.drive.helper.print(error_prefix + 'Facebook user-page unavailable')
            ...
            if await self.drive.await_selectors(selector['follow'], self.page, timeout=5, silent=True):
               for index in range(3):  # Retry logic with hover only on the first attempt
                   await self.drive.click_btn(selector['follow'], self.page, silent=True, hover = True if index == 0 else False)

                   # Break early if "Following" button appears (follow was successful)
                   if await self.drive.await_selectors(selector['following'], self.page, silent=True, timeout=3):
                      break

            # Final check: confirm user was successfully followed
            if await self.drive.await_selectors(selector['following'], self.page, silent=True):
               return True  # Follow succeeded

      async def demo(self):
            url = 'https://web.facebook.com/profile.php?id=61578820910096'
            await self.drive.helper.site_lamba(url, self.page, silent=True)
            print (await self.page_like())
